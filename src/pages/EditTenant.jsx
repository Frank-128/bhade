...
useEffect(()=>{
 (async()=>{
 await axios.get("getTenant/3").then(res=>{
 setFirstname(res.data.tenant.firstname);
 setLastname(res.data.tenant.lastname);
 setPhoneNo(res.data.tenant.phoneNumber);
 setMetre(res.data.tenant.currMetreReading);
 setAmountPaid(res.data.tenant.amountPaid);
 setContract(res.data.tenant.contract);
 setEndDate(res.data.tenant.endDate);
 setStartDate(res.data.tenant.startDate);
 setRoomNo(res.data.tenant.roomNo);
 ...
 const addTenant= (e)=>{
  ...
  axios.put("updateTenant/3", {firstname,lastname,phoneNo,metre, roomNo,amountPaid, startDate, endDate, contract}).then(res=>{
    console.log(res)
  });
   ...
   <div className="flex  bg-slate-600  justify-center ml:h-screen al:h-full ">
     ...
     <form onSubmit={addTenant} className=" flex  ml:w-2/3 al:w-screen  h-fit ml:my-3 pb-2 ml:rounded-2xl bg-slate-400  flex-col items-center justify-center ">
      ...
      <div className="mb-6">
        ...
        <button onClick={()=>viewFile("http://localhost:8000/storage/"+contract)}> viewFile</button>
        <input
          type="file"
